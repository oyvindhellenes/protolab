...
// [START subscribe customer]
// Charge the Stripe customer whenever an amount is written to the Realtime database
exports.createStripeSubscribe = functions.database.ref('/stripe_customers/{userId}/subscriptions/{id}').onWrite(event => {
  const val = event.data.val();
  // This onWrite will trigger whenever anything is written to the path, so
  // noop if the charge was deleted, errored out, or the Stripe API returned a result (id exists)
  if (val === null || val.id || val.error) return null;
  ...
  return admin.database().ref(`/stripe_customers/${event.params.userId}/customer_id`).once('value').then(snapshot => {
    ...
    // Create a charge using the pushId as the idempotency key, protecting against double charges
    const customer = val.id;
    const plan = "protolab-vest-subscription";
    const idempotency_key = event.params.id;
    let subscribe = {customer, plan};
    if (val.source !== null) subscribe.source = val.source;
